...
dataName_city = 'g11_city_data.bag';
filename_city = [path, dataName_city];
if ~exist('city_imu_existFlag')
    %get rosbag
...
%     imuData_city = readMessages(imuBag);
    ...
    save([path, 'imu_city.mat'], 'angVel_city', 'LinAcc_city');
    city_imu_existFlag = 1;
end

if ~exist('city_gps_existFlag')
    %get rosbag
    bag_stat = rosbag(filename_city);
    %select o
    ...
    %get angular velocity into matrix form
    save([path, 'gps_city.mat'], 'gps_fix', 'gps_vel');
